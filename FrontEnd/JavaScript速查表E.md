...
| 数据类型        | 例子                                      |
| ----------- | --------------------------------------- |
| `undefined` | 声明但未赋值的变量的类型是 `undefined`.              |
| `null`      | 没有声明的类型，即变量不存在                          |
...
- ？？？与 `var`不同, `let` 声明一个变量两次会抛出异常.
- 在块、语句或表达式中声明的变量`let`，其范围仅限于该块、语句或表达式.
- `const` 修饰的变量为不可变变量，即只读，且不能被再次赋值.
- 如果`const` 修饰的变量的值是一个对象（包括数组和函数），则变量不能指向新的对象，但可以修改对象的内容。

为了确保数据不会改变，JavaScript 提供了 Object.freeze 函数来防止数据改变。
...
Object.freeze(obj);
obj.review = "bad"; // 不被执行，不允许修改
obj.newProp = "Test"; // 不被执行，不允许修改
console.log(obj);
