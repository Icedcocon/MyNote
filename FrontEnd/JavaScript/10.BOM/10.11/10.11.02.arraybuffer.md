...
- **(1) Blob(Binary Large Object) 对象表示一个二进制文件的数据内容，如图片或文件。**
- **(2) 与 ArrayBuffer 的区别在于，它用于操作二进制文件，而 ArrayBuffer 用于操作内存。**
- **(3) `Blob`构造函数接受两个参数。**
  - **第一个参数是数组，成员是字符串或二进制对象，表示新生成的`Blob`实例对象的内容；**
  - **第二个参数是可选的配置对象，只有属性`type`值是默认为空的字符串，说明 MIME 类型。**
- **(4) `Blob`具有两个实例属性`size`和`type`，分别返回数据的大小和类型。**
  - **`myBlob.size // 32`**
  - **`myBlob.type // "text/html"`**
- **(5) `Blob`具有一个实例方法`slice`，用来拷贝原来的数据，返回的也是一个`Blob`实例。**
  - **`myBlob.slice(start, end, contentType)`**

---

- **(1) 文件选择器`<input type="file">`让用户选取文件，脚本无法设置控件的`value`属性。**
  - **返回一个 FileList 类数组对象，成员都是 File 实例对象，是一个特殊的 Blob 实例。**
  - **`// <input type="file" accept="image/*" multiple onchange="fileinfo(this.files)"/>`**
  - **AJAX 请求时，如果指定`responseType`属性为`blob`，下载下来的就是一个 Blob 对象。**
- **(2) 用`URL.createObjectURL()`方法可为 Blob 对象生成一个临时 URL 以便 API 使用。**
  - **URL 以`blob://`开头，协议头后面是一个识别符对应内存里面的 Blob 对象。**
  - **与`data://URL`（URL 包含实际数据）和`file://URL`（本地文件系统里面的文件）都不一样。**
- **(3) `FileReader`对象可读取 Blob 对象的内容。**
