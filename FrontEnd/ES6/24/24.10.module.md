...
- **(1) `import(specifier)` 参数`specifier`指定要加载模块的位置。**
- **(2) `import`命令接受参数与`import()`函数一致，区别主要是后者为动态加载。**
- **(3) `import()`返回一个 Promise 对象，需用`then()`方法指定处理函数，推荐用`await`命令代替。**
  - **`import(`./${var}.js`).then(module => {module.loadPageInto(main);})`**
  - **`const widget = await import('./widget.js');widget.render(container);`**
- **(4) `import()`加载模块成功后，模块作为对象当作`then`方法的参数,可用对象解构赋值获取输出接口。**
  - **`import('./myModule.js').then(({export1, export2}) => {// ...});`**
- **(5) 模块有`default`输出接口或使用具名输入的形式。**
