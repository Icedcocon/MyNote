## 对象的扩展-AggregateError 错误对象-总结

- **(1) ES2021 为配合新增的`Promise.any()`方法引入一个新的错误对象`AggregateError`**
- **(2) AggregateError 在一个错误对象里面，封装了多个错误。**
- **(3) 单一操作同时引发了多个错误，需要同时抛出这些错误时可以抛出一个 AggregateError 错误对象**

## 对象的扩展-AggregateError 错误对象
