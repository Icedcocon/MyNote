...
- **(1) 每个 Vue 组件实例在创建时都需要经历一系列的初始化步骤**
  - **如设置好数据侦听，编译模板，挂载实例到 DOM，以及在数据改变时更新 DOM。**
  - **此过程中也会运行被称为生命周期钩子的函数以运行自定义代码。**
- **(2) 如 `onMounted` 钩子可以用来在组件完成初始渲染并创建 DOM 节点后运行代码**
- **(3) 常用钩子函数有 **
  - **`onMounted` `onUpdated` `onUnmounted` **
  - **`mounted` `updated` `unmounted`**
- **(4) 生命周期钩子函数的 `this` 上下文都会自动指向当前调用它的组件实例。**
  - **注意：避免用箭头函数来定义生命周期钩子，防止无法获取组件实例。**
- **(5) 钩子应当在组件初始化时被同步注册，不要使用 `setTimeout` 函数注册**
