# LeC.0862.Shortest-Subarray-with-Sum-at-Least-K
...
func shortestSubarray(nums []int, k int) int {
    var prefixSums []int = make([]int, len(nums)+1)
    for i := 0; i < len(nums); i++ {
        prefixSums[i+1] = prefixSums[i] + nums[i]
    }
    res, queue := len(nums)+1, []int{}
    for i, num := range prefixSums {
        for len(queue) > 0 && num-prefixSums[queue[0]] >= k {
            res = min(res, i-queue[0])
            queue = queue[1:]
        }
        for len(queue) > 0 && num <= prefixSums[queue[len(queue)-1]] {
