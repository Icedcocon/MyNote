# testing - 子测试与子基准测试-总结

- **(1) T 和 B 的 `Run` 方法允许定义子单元测试和子基准测试而无需单独定义函数。**
- **(2) 便于创建基于 Table-Driven 的基准测试和层级测试和共享通用 `setup` 和 `tear-down` 代码。**
- **(3) 子(基准)测试有唯一的名称：由顶层测试名称与`Run`的名称参数组成，以斜杠分隔且有可选尾随序列号以消除歧义。**
- **(4) 在所有子测试并行运行完毕之前，`Run` 方法不会返回，可用于并行控制/清理资源。**
- **(5) `t.Run(tc.Name, func(t *testing.T) {t.Parallel()...})` 子测试间并行运行，顶层其它没有 `t.Parallel()` 的子测试不受影响**
