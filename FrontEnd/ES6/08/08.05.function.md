...
- **(2) 箭头函数不需要参数或需要多个参数可使用一个圆括号代表参数部分。**
  - **`var f = () => 5;` 等同于 `var f = function () { return 5 };`**
  - **`var sum = (num1, num2) => num1 + num2;`**
- **(3) 箭头函数的代码块部分多于一条语句需用大括号括起来并使用`return`语句返回。**
  - **`var sum = (num1, num2) => { return num1 + num2; }`**
- **(4) 如果箭头函数直接返回一个对象，必须在对象外面加上括号，否则会报错。**
  - **`let getTempItem = id => ({ id: id, name: "Temp" });`**
- **(5) 如果箭头函数只有一行语句且无返回值，可以采用下面的写法，就不用写大括号了。**
  - **`let fn = () => void doesNotReturn();`**
- **(6) 箭头函数可以与变量解构结合使用。**
  - **`const full = ({ first, last }) => first + ' ' + last;`**
- **(7) 箭头函数可简化回调函数。**
  - **`[1,2,3].map(x => x * x);` 等价于 `[1,2,3].map(function (x) {return x * x;});`**
...
- **(0)箭头函数使得`this`从“动态”变成“静态”，下面两个场合不应该使用箭头函数。**

- **(1)第一个场合是定义对象的方法，且该方法内部包括`this`。**
  
  - **`const cat = {lives: 9,jumps: () => {this.lives--;}}`**
  - **如果jumps是普通函数，该方法内部的`this`指向`cat`，而箭头函数的`this`指向全局对象，因为对象不构成单独的作用域。**
...
- **(2)第二个场合是需要动态`this`的时候，也不应使用箭头函数。**
  ...
  - **`button`的监听函数是一个箭头函数，导致里面的`this`就是全局对象，而普通函数会动态指向被点击的按钮对象。**
