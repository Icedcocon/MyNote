...
- **(1) 尾调用（Tail Call）是指某个函数的最后一步是调用另一个函数。**
- **(2) `function f(x){return g(x);}`**
- **(3) 尾调用不一定出现在函数尾部，只要是最后一步操作即可。**

---

- **(1) ES6 的尾调用优化只在严格模式下开启，正常模式是无效的。**
- **(2) 因为在正常模式下，函数内部有两个变量，可以跟踪函数的调用栈。**
  - **`func.arguments`：返回调用时函数的参数。**
  - **`func.caller`：返回调用当前函数的那个函数。**
- **(3) 内层函数中不再使用外层函数的内部变量时（参数可用），内层函数调用帧才会取代外层函数调用帧。**
- **(4) 目前只有 Safari 浏览器支持尾调用优化。**

---

- **(1) 递归需要保存上百个调用帧，容易发生“栈溢出”错误（stack overflow）。**
- **(2) 尾递归来只存在一个调用帧，所以不会发生“栈溢出”错误。**
