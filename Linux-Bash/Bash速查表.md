...
bash file.sh        # 在子shell中运行bash脚本
./file.sh           # 在子shell中运行bash脚本
source file.sh      # 在当前shell中运行bash脚本
. file.sh           # 在当前shell中运行bash脚本
which bash          # 搜索 $PATH，查找哪个程序对应命令 bash
whereis bash        # 搜索可执行，头文件和帮助信息的位置，使用系统内建数组据库
...
dirs -c             # 清空目录栈
cd -                # 回到之前的目录
pushd +{N}          # 切换到目录栈中的第 N个目录，pushd -{N}倒数第N个
...
ls                  # 显示当前目录内容，ls {dir}显示指定目录，ls -al显示隐藏文件
...
diff <(cmd) <(cmd)  # 比较两个命令输出的内容

wc {fn}             # 统计文件有多少行，多少个单词 -l 仅行数 -w 仅单词数
file {fn}           # 检测文件的类型和编码
stat {fn}           # 显示文件的详细信息
...
chown user {fn}     # 修改文件所有人为 user
chown user:group fn # 同时修改文件所属组为group，所有人为 user
```
...
useradd {user}      # 添加用户
userdel -r {user}   # 删除用户，-r 删除家目录所有文件及邮箱
su                  # 切换到 root 用户
...
declare -A array                     # 关联数组需要先声明再使用
array=([name]=fry [sex]=m [title]=e) # 关联数组可以使用字符作为索引
...
num=$(expr 1 + 2)         # (1)兼容 posix sh 的计算，使用 expr 命令计算结果
num=$(expr $num + 1)      # 数字自增
expr 2 \* \( 2 + 3 \)     # 兼容 posix sh 的复杂计算，输出 10

num=$((1 + 2))            # (2)计算 1+2 赋值给 num，使用 bash 独有的 $((..)) 计算
num=$[1+2]                # (3)同上
num=$(($num + 1))         # 变量递增
num=$((num + 1))          # 变量递增，双括号内的 $ 可以省略
num=$((1 + (2 + 3) * 2))  # 复杂计算

let num=num+2             # (4)第四种计算方式
let num++                 # 自增
...
awk 'BEGIN{...}{...}END{...}' file # 行处理前（无需文件）、中（每行一次）、后
awk 'BEGIN{print ""Hello}'         # 打印字符串（放在双引号中）
awk '{print $5,$6}' file           # 打印文件中以空格分隔的第五、六列
awk -F ',' '{print $5}' file       # 打印文件中以逗号分隔的第五列
awk 'BEGIN{FS=","} {print $5}' file# 另一种方式{FS分割符，OFS输出分隔符}
awk '/str/ {print $2}' file        # 打印文件中包含 str 的所有行的第二列
awk -F ',' '{print $NF}' file      # 打印逗号分隔的文件中的每行最后一列 
awk '{s+=$1} END {print s}' file   # 计算所有第一列的合
awk 'NR%3==1' file                 # 从第一行开始，每隔三行打印一行
awk 'printf "%-5s\n",$3'           # 左对齐5字符间隔打印文件中以空格分隔的第3列
awk '/^\s*$/{print NR}'            # 打印只有空白字符（\f\n\r\t\v）的行号
awk '!/keywords/'                   # 查找所有不匹配keywords的行
awk '$NF!~/bash$/' file            # 查找最后一列中不是以bash结尾的行
awk "gsub(/old/,\"$new\")" file    # 使用外部变量$new替换old字符串
awk 'gsub(/old/,"'"$new"'")' file' # 另一种写法
awk 'if(int($1)>'''$var'''){...}'  # 在{}外面可以用三个单引号
awk '{for(i=1;i<=NF;i++){arr[$i]++}}\
END{for(word in arr)\
{print word, arr[word]}}'          # 打印词频
...
sed -r 's/regex/replace/gi' file   # 替换文件中所有出现的字符串（i忽略大小写）
...
```bash
##############################################################################
# 网络管理：ip（iproute） / ifconfig（net-tools） / nmap ...
##############################################################################
...
ip netns delete {ns}               # 删除
ip netns add {ns}                  # 创建
ip netns exec {ns} ip a            # 在namespace中执行指令 ip a
ip link set dev {lo} up            # 让换回设备运行
ip link add {veth1} type veth peer name {veth2} # 添加一对veth接口
ip link set {veth} netns {ns}      # 将本网络命名空间的veth端口添加到{ns}空间中
ip addr add 192.168.1.1/24 dev {veth1} # 为端口配置IP地址
ip link                            # 查看连接情况
