...
- **(2) `type="search"`是一个用于搜索的文本输入框，基本等同于`type="text"`。**
- **(3) `type="button"`是没有默认行为的按钮，通常脚本指定`click`事件的监听函数来使用。**
  - **使用语义更清晰，且可以插入图片或其他 HTML 的`<button>`标签代替。**
- **(4) `type="submit"`是表单的提交按钮。用户点击这个按钮，就会把表单提交给服务器。**
  - **`<input type="submit" value="提交">`**
  - **不指定`value`属性，浏览器会在提交按钮上显示默认的文字，通常是`Submit`。**
...
- **(5) `type="image"`表示将一个图像文件作为提交按钮，行为和用法与`type="submit"`完全一致。**
...
- **(7) `type="checkbox"`是复选框，允许选择或取消选择该选项。**
  - **同时选中两个复选框，提交时有两个`name`属性，如`n=coding & n=music`。**
- **(8) `type="radio"`是单选框，表示一组选择之中，只能选中一项。**
  - **多个单选框的`name`属性的值，应该都是一致的。**
  - **`checked`：布尔属性，表示是否默认选中当前项。**
  - **`value`：用户选中该项时，提交到服务器的值，默认为`on`。**
- **(9) `type="password"`是一个密码输入框。**
  ...
  - **`autocomplete`：是否允许自动填充**
    - **`on`（允许）、`off`（不允许）**
    - **`current-password`（填入当前网站保存的密码）**
    - **`new-password`（自动生成一个随机密码）。**
  - **`inputmode`：允许用户输入的数据类型，可能的值有**
    - **`none`（不使用系统输入法）**
    - **`text`（标准文本输入）**
    - **`decimal`（数字，包含小数）**
    - **`numeric`（数字0-9）等。**
- **(10) `type="file"`是一个文件选择框，允许用户选择一个或多个文件，常用于文件上传功能。**
  - **`accept`：允许选择的文件类型，使用逗号分隔**
    - **可以使用 MIME 类型（比如`image/jpeg`）**
    - **也可以使用后缀名（比如`.doc`）**
    - **还可以使用`audio/*`（任何音频）、`video/*`（任何视频）、`image/*`（任何图像）**
  - **`capture`：用于捕获图像或视频数据的源，可能的值有**
    - **`user`（面向用户的摄像头或麦克风）**
