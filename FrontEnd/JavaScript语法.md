...
// 定义
'abc'                      // (1)零个或多个排在一起的字符，放在单引号
"abc"                      //    或双引号之中
'key = "value"'            // (2)单引号字符串的内部，可以使用双引号
"It's a long journey"      //    双引号字符串的内部，可以使用单引号
'Did she say \'Hello\'?'   // (3)单引号字符串的内部使用单引号，必须转义
"Did she say \"Hello\"?"   //    双引号字符串的内部使用单引号，必须转义
// (4)由于HTML语言的属性值使用双引号，所以约定 JavaScript 语言的字符串只使用单引号
var longString = 'Long \
long \
long \
string';                   // (5)多行字符串，可在行尾使用反斜杠，但输出还是单行
var longString = 'Long '
  + 'long '
  + 'long '
  + 'string';              // (6)运算符（+）可连接多个单行字符串，效果同上
(function () { /*
line 1
line 2
line 3
...
console.log('1\n2')       // 输出多行字符串
//反斜杠有三种特殊用法:
'\251'   // "©" (1) 反斜杠后面紧跟三个八进制数（000到377），代表一个字符。
'\172' === 'z'     // true
'\xA9'   // "©" (2) \x后面紧跟两个十六进制数（00到FF），代表一个字符。
'\x7A' === 'z'     // true
'\u00A9' // "©" (3) \u后面紧跟四个十六进制数（0000到FFFF），代表一个字符。
'\u007A' === 'z'   // true
'\a'     // "a" (4) 在非特殊字符前面使用反斜杠，则反斜杠会被省略。

// 字符串与数组
var s = 'hello'; s[0]             // (1) "h" 字符串可以被视为字符数组
'hello'[1]                        //     "e" 直接对字符串使用方括号运算符
'abc'[3]                          // (2) undefined 若方括号中数字大于长度
'abc'[-1]                         //     undefined 或者方括号中根本不是数字
'abc'['x']                        //     undefined 则返回undefined
delete s[0];                      // (3) "hello" 无法改变字符串之中的单个字符

// length 属性
s.length                          // (1) 5 length属性返回字符串的长度
s.length = 3;                     // (2) 5 length属性无法改变，但是不会报错。


// #############################################################
// 字符集
// #############################################################
// JavaScript 使用 Unicode 字符集。JavaScript 不仅以 Unicode 储存字符。
var s = '\u00A9';            // (1) JS 允许在程序中用 Unicode 码点表示字符
var f\u006F\u006F = 'abc';   // (2) 解析代码时，JS会自动识别字面/Unicode形式
foo                          // (3) "abc" 输出时，所有字符都会转成字面形式。
// (4) 每个字符在 JavaScript 内部都是以16位（即2个字节）的 UTF-16 格式储存。
//     也就是说，JavaScript 的单位字符长度固定为16位长度，即2个字节。
// (5) UTF-16 有两种长度:16位（即2个字节）/ 32位（即4个字节）JS只支持两字节的字符
'𝌆'.length                  // 2 JavaScript 认为𝌆的长度为2，而不是1
//     对于码点在U+10000到U+10FFFF之间的字符，JS认为是两个字符（length属性为2）

// #############################################################
// Base64 转码
// #############################################################
// (1) 文本包含不可打印符号(如 ASCII 码0到31)，可用 Base64 编码转成可以打印的字符。
// (2) 以文本格式传递二进制数据，可以使用 Base64 编码。
// (3) Base64 是一种编码方法，可将任意值转成 0～9、A～Z、a-z、+和/组成的可打印字符
// (4) JavaScript 原生提供两个 Base64 相关的方法。
var string = 'Hello World!';
btoa(string)                 // (1) "SGVsbG8gV29ybGQh"任意值转为Base64编码
atob('SGVsbG8gV29ybGQh')     // (2) "Hello World!"    Base64编码转为原来值
btoa('你好')                  // (3) 报错 这两个方法不适合非 ASCII 码的字符
function b64Encode(str) {    // (4) 非 ASCII 码字符转为 Base64 编码
  return btoa(encodeURIComponent(str));  // 中间插入一个转码环节
}
...
// 生成方法
var obj = {        // 大括号定义一个对象，它被赋值给变量obj
  foo: 'Hello',    // 对象内部包含两个键值对，foo是“键名”，Hello是“键值”
  bar: 'World'     // 键名与键值之间用冒号分隔，两个键值对之间用逗号分隔。
};
// 键名
var obj = {        // (1) 对象的所有键名都是字符串（ES6 引入Symbol值）
  'foo': 'Hello',
  'bar': 'World'
};
var obj = {        // (2) 
